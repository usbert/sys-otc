...
class UpdateSupplyerRequest extends FormRequest {
    ...
    {
        return [
            'name'            => 'required|string|min:3|max:80|unique:supplyers,name,'.$this->id,
            'fantasy_name'    => 'required|string|min:3|max:80',
            'document_number' => 'required|string|min:1|max:18|unique:supplyers,document_number,'.$this->id,
        ];
    }
